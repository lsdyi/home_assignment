...
# check sample mean of Cauchy
cauchy_draws <- rcauchy(10000, 0)
sample_mean_list <- rep(NA, length(cauchy_draws))
for (i in 1:length(cauchy_draws)) {
    sample_mean_list[i] <- mean(cauchy_draws[1:i])
}
plot(sample_mean_list, type = "l")
```
...
### c. Construct at 95% credible interval for θ.
...
cdf <- cumsum(postDen * binSize)
plot(thetaGrid, cdf)
lower_index <- max(which(cdf <= 0.025))
upper_index <- max(which(cdf <= 0.975))
thetaGrid[lower_index]
thetaGrid[upper_index]
...
plot(sampleMeanList, type = "l", xlab = "sample size")
...
LogPostBetaReg <- function(Beta, X, y, mu_0 = 0, sigma2_0 = 10^2) {
    ...
    phi <- 4
    logLikeli <- sum(dbeta(y, mu * phi, (1 - mu) * phi, log = TRUE))
...
phi <- 4
y_tilde <- rbeta(length(mu * phi), mu * phi, (1 - mu) * phi)
