...
for (i in 1:length(postMode)) {
    mu <- postMode[i]
    sigma2 <- postCov[i, i]
    thetaGrid <- seq(mu - 10 * sqrt(sigma2), mu + 10 * sqrt(sigma2), length = 1000)
    bin_size <- thetaGrid[2] - thetaGrid[1]

    density_list <- dnorm(thetaGrid, mean = mu, sd = sqrt(sigma2))
    plot(thetaGrid, density_list, type = "l")
    # print(sum(density_list * bin_size)) # check if covers area

    # equal tail: 95%
    cdf <- cumsum(density_list * bin_size)
    lower <- tail(thetaGrid[cdf < 0.025], 1)
    upper <- tail(thetaGrid[cdf < 0.975], 1)
...
RWMsampler <- function(logPostFunc, initVal, nSim, nBurn, Sigma, c, ...) {
    ...
    return(list(
        draws = draws[(nBurn + 1):nSim, ],
        alpha_list = alpha_list[(nBurn + 1):nSim]
    ))
}
```
...
c <- 0.5
Sigma <- postCov
res <- RWMsampler(LogPostPoissonReg, inital_value, nSim, nBurn, Sigma, c, X, y, mu_0 = mu_0, sigma_0 = sigma_0)
draws = res$draws
head(draws, 20)
...
```{r}
res <- RWMsampler(LogPostPoissonReg, rep(0, 5), nSim, nBurn, Sigma = diag(5), c = 1, X, y, mu_0 = mu_0, sigma_0 = sigma_0)
new_covariance_draws = res$draws
