...
### (a) predictive distribution follows Poisson(5)
```{r}
n_draws <- 1000
pred_draws <- rpois(n_draws, 5)
mean(pred_draws)
hist(pred_draws, col = "lightgray", freq = FALSE)

a_grid <- 0:20
utility_list <- rep(NA, length(a_grid))
for (i in 1:length(a_grid)) {
    utility_list[i] <- mean(utility(pred_draws, a_grid[i]))
}

plot(a_grid, utility_list)
maximizer <- a_grid[which.max(utility_list)]
abline(v = maximizer, lty = 2, col = "red")
maximizer
```

### (b) predictive distribution
Poisson Model with Gamma prior, the posterior follows $Gamma(n\bar{x}+\alpha, n+\beta)$. Getting $\theta$ draws from gamma simulation.
```{r}
data <- c(3, 6, 9, 4, 5)
alpha_0 <- 5
beta_0 <- 1
alpha_n <- sum(data) + alpha_0
beta_n <- length(data) + beta_0
n_draw = 1000
draws = rgamma( n_draw, alpha_n, beta_n)

# get y_tilde draws
pred_draws = rpois(n_draw, draws)
